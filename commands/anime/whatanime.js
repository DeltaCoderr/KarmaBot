    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (message.attachments.first()) {
                ...
                screenshot = user.user.displayAvatarURL({
                    format: 'png'
                })
            } else {
                screenshot = message.author.displayAvatarURL({
                    format: 'png'
                })
            }
...
async function fetchRateLimit() {
    try {
        const {
            body
        } = await request.get('https://trace.moe/api/me');
        return {
            status: body.user_limit > 0,
            refresh: body.user_limit_ttl
        };
    } catch {
        return {
            status: false,
